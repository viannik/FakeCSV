...
def generate_data(data_type, **kwargs):
	...
	match data_type:
		...
		case 'Company Name':
			return fake.company()
		case 'Text':
			num_sentences = random.randint(kwargs.get('from_int'), kwargs.get('to_int'))
			return ' '.join(fake.sentences(num_sentences))
		case 'Integer':
			return random.randint(kwargs.get('from_int'), kwargs.get('to_int'))
		case 'Address':
			return fake.address()
		case 'Date':
			return fake.date()
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			with tempfile.NamedTemporaryFile(mode='w+', delete=False) as temp_file:
				file_name = temp_file.name  # Save file name to variable
				writer = csv.writer(temp_file, delimiter=separator, quotechar=character, quoting=csv.QUOTE_ALL)
				writer.writerow([column.name for column in columns])
				for i in range(rows):
					row = [str(
						generate_data(column.column_type, from_int=column.from_int, to_int=column.to_int))
						for column in columns]
					writer.writerow(row)

			with open(file_name, 'r') as f:
				csv_file = CSVFile(user=request.user, schema=schema)
				csv_file.file.save(f'{schema.name}-{rows}.csv', f)

			os.unlink(file_name)
			return redirect('schema_datasets', pk=schema.pk)  # Reload the page
	else:
		form = CSVFileForm()
